...
private fun String.heightMap(): HeightMap =
    File(this).readLines().map { it.toList().map { Character.getNumericValue(it) } }
...
fun main() {
    val heightMap = "d09.in".heightMap()
    var part1 = heightMap.lowPoints().sumOf { (x, y) -> heightMap[x][y] + 1 }
    var part2 = heightMap.lowPoints().map { (rowIdx, colIdx) -> heightMap.basin(rowIdx, colIdx).toSet().size }
